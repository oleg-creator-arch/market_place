...
//		$redirect_url = isset( $_SERVER['REDIRECT_URL'] ) ? $_SERVER['REDIRECT_URL'] : $redirect_url;
...
//			} elseif ( $old_currency && $old_currency != $current_currency && $id ) {
//				$this->settings->set_current_currency( $old_currency, false );
//			}
		} else {
			if ( $settings->enable_cart_page() && is_cart() || $is_checkout ) {
				$settings->set_current_currency( $checkout_currency, false );

				return;
			}

			if ( $old_currency && $old_currency != $current_currency ) {
...
//				if ( $id ) {
//					if ( $allow_multi && $id == get_option( 'woocommerce_checkout_page_id', 0 ) ) { // is checkout page
