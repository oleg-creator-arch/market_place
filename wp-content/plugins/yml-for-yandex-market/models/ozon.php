 ...
 if ($product->is_type('variable')) {
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		*/
		...
		$offer_id_yml = 'id="'.$offer_id_yml.'"';
		$offer_id_yml = apply_filters('yfym_variable_offer_id_yml_filter', $offer_id_yml, array($offer_id, $product, $offer, $offer_id_yml), $numFeed);
		$result_yml .= '<offer '.$offer_type.$gi.' '.$offer_id_yml.' available="'.$available.'" '.$append_offer_tag.'>'.PHP_EOL;		
...
//			}
			$result_yml .= '<outlets>'.PHP_EOL;
			$result_yml .= '<outlet instock="'.htmlspecialchars($stock_quantity).'" warehouse_name="Основной склад"></outlet>'.PHP_EOL;
			$result_yml .= '</outlets>'.PHP_EOL;
		} else {
...
//				}
 ...
 $offer_id_yml = 'id="'.$post_id_yml.'"';
 $offer_id_yml = apply_filters('yfym_simple_offer_id_yml_filter', $offer_id_yml, array($postId, $product, $post_id_yml), $numFeed);
 $result_yml .= '<offer '.$offer_type.' '.$offer_id_yml.' available="'.$available.'" '.$append_offer_tag.'>'.PHP_EOL;
